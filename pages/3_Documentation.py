...
def documentation_page():
    ...
    #### Механика баллов и комиссий
    - **Начисление и использование баллов**: 
        - Баллы начисляются автоматически при покупке
        - Требуется подтверждение (claim) пользователем в течение 2 месяцев
        - Неподтвержденные баллы возвращаются в систему
        - В стандартном сценарии 55% начисленных баллов используются пользователями
    
    - **Сгорание баллов**:
        - Из оставшихся 45% неиспользованных баллов:
            * 7% сгорают через 12 месяцев
            * Сгоревшие баллы учитываются как доход платформы
    
    #### Доходы платформы
    - **Партнерский доход**:
        - Комиссия 0.5% от общего оборота (GMV)
